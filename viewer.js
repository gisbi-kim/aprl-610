...
const modelFileBytes={"mvs-balanced.glb": 11494388, "mvs-complete.glb": 11669628, "mvs-hybrid-additions.glb": 3729080, "mvs-hybrid-more-additions.glb": 3852604, "mvs-hybrid-more.glb": 18071616, "mvs-hybrid.glb": 17025396, "mvs-lingbot-bundle.glb": 17198756, "mvs-lingbot-larger.glb": 18997116, "mvs-lingbot-only.glb": 17105028, "mvs-lingbot-repaired.glb": 18808136, "mvs.glb": 11219568, "surface": 20430652, "brush": 8704841};
function modelSize(bytes){return (bytes/1e6).toFixed(2)+" MB";}
function updateTabSize(mode,bytes){if(bytes)$("#model-"+mode+" .model-size").textContent=modelSize(bytes);}
const mvsCache=new Map();let mvsAsset='mvs-hybrid-more.glb';
const mvsPanel=document.createElement('section');mvsPanel.id='mvsControls';mvsPanel.hidden=true;
mvsPanel.innerHTML='<label for="mvsVariant">MVS 비교</label><select id="mvsVariant" style="width:100%;padding:9px;border:1px solid #dde3de;border-radius:9px;background:#f7f8f4;color:#4e625d"><option value="mvs-hybrid-more.glb" selected>확장 혼합 · LingBot 더 많이 · 18.07 MB</option><option value="mvs-hybrid.glb">혼합 · 보수적 보완 · 17.03 MB</option><option value="mvs-lingbot-bundle.glb">LingBot · GPU BA + 국소 보정 · 17.20 MB</option><option value="mvs-lingbot-larger.glb">LingBot only · 더 큰 구멍 보정 (실험) · 19.00 MB</option><option value="mvs-lingbot-repaired.glb">LingBot only · 작은 구멍 보정 · 18.81 MB</option><option value="mvs-lingbot-only.glb">LingBot only · 표면 · 17.11 MB</option><option value="mvs.glb">기존 · 11.22 MB</option><option value="mvs-balanced.glb">보완 · 중간 완화 · 11.49 MB</option><option value="mvs-complete.glb">비교 · 강한 완화 · 11.67 MB</option></select><label style="display:block;margin-top:12px">채도 <output id="mvsSaturationValue">65%</output><input id="mvsSaturation" type="range" min="0" max="1.2" step="0.05" value="0.65" style="width:100%"></label><label id="hybridHighlightRow"><input type="checkbox" id="hybridHighlight">LingBot 보완 부분 표시</label><p id="mvsCompareNote" style="font-size:11px;line-height:1.6;color:#7d8580">같은 깊이맵에서 표면 융합 조건을 비교합니다. 큰 미관측 영역은 남아 있습니다.</p>';
...
gsPanel.innerHTML='<label for="gsVariant">GS 비교</label><select id="gsVariant" style="width:100%;padding:9px;border:1px solid #dde3de;border-radius:9px;background:#f7f8f4;color:#4e625d"><option value="surface">2DGS · 표면 제약 · 20.43 MB</option><option value="brush">기존 3DGS · Brush · 8.70 MB</option></select><p id="gsStats" style="font-size:12px;line-height:1.6">Gaussian Splatting<br>LingBot 초기화 · 사진 24장<br>6,000단계 · 436,961 splats</p><label>불투명도 <output id="gsOpacityValue">1.0</output><input id="gsOpacity" type="range" min="0.1" max="1" step="0.05" value="1" style="width:100%"></label><p style="font-size:11px;color:#7d8580;line-height:1.6">사진으로 학습한 시각 표현입니다. 빈 영역과 잔상은 남을 수 있으며 충돌용 표면은 아닙니다.</p>';
...
function applyMode(next){mode=next;for(const [id,item] of extraGS)item.mesh.visible=mode==='gs'&&gsVariant===id;if(gs)gs.visible=mode==='gs'&&gsVariant==='brush';if(surfaceGS)surfaceGS.visible=mode==='gs'&&gsVariant==='surface';$('#gsControls').hidden=mode!=='gs';cute.visible=mode==='cute';if(scan)scan.visible=mode==='scan';if(lingbot)lingbot.visible=mode==='lingbot';ground.visible=mode==='cute';
