...
function applyMode(next){if(next==='gs'&&mode!=='gs')preset('entry');mode=next;for(const [id,item] of extraGS)item.mesh.visible=mode==='gs'&&gsVariant===id;if(gs)gs.visible=mode==='gs'&&gsVariant==='brush';if(surfaceGS)surfaceGS.visible=mode==='gs'&&gsVariant==='surface';$('#gsControls').hidden=mode!=='gs';cute.visible=mode==='cute';if(scan)scan.visible=mode==='scan';if(lingbot)lingbot.visible=mode==='lingbot';ground.visible=mode==='cute';
...
function resize(){const mobile=innerWidth<=720,width=mobile?innerWidth:Math.max(300,innerWidth-290),bottom=mobile?innerHeight-$('aside').getBoundingClientRect().top+14:45,top=Math.ceil($('header').getBoundingClientRect().bottom)+(mobile?12:18),height=Math.max(150,innerHeight-bottom-top);camera.aspect=width/height;camera.updateProjectionMatrix();renderer.setSize(innerWidth,innerHeight);renderer.setViewport(0,bottom,width,height);$('footer').style.bottom=(mobile?bottom+4:24)+'px';}
