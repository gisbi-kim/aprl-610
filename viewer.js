...
const mvsCache=new Map();let mvsAsset='mvs-hybrid.glb';
const mvsPanel=document.createElement('section');mvsPanel.id='mvsControls';mvsPanel.hidden=true;
mvsPanel.innerHTML='<label for="mvsVariant">MVS 비교</label><select id="mvsVariant" style="width:100%;padding:9px;border:1px solid #dde3de;border-radius:9px;background:#f7f8f4;color:#4e625d"><option value="mvs-hybrid.glb" selected>혼합 · MVS + LingBot</option><option value="mvs.glb">기존</option><option value="mvs-balanced.glb">보완 · 중간 완화</option><option value="mvs-complete.glb">비교 · 강한 완화</option></select><label id="hybridHighlightRow"><input type="checkbox" id="hybridHighlight">LingBot 보완 부분 표시</label><p id="mvsCompareNote" style="font-size:11px;line-height:1.6;color:#7d8580">같은 깊이맵에서 표면 융합 조건을 비교합니다. 큰 미관측 영역은 남아 있습니다.</p>';
...
let hybridOverlay,hybridOverlayPromise;
async function updateHybridOverlay(){
 const wanted=()=>mode==='scan'&&mvsAsset==='mvs-hybrid.glb'&&$('#hybridHighlight').checked;
 if(hybridOverlay)hybridOverlay.visible=wanted();
 if(!wanted())return;
 if(!hybridOverlayPromise)hybridOverlayPromise=new Promise((resolve,reject)=>loader.load('mvs-hybrid-additions.glb?v=2',g=>{hybridOverlay=g.scene;hybridOverlay.traverse(o=>{if(o.isMesh){o.material=new THREE.MeshBasicMaterial({color:0xf5a63a,side:THREE.DoubleSide,transparent:true,opacity:.85,depthWrite:false,polygonOffset:true,polygonOffsetFactor:-2,polygonOffsetUnits:-2,clippingPlanes:$('#cutaway').checked?[roofClip]:[]});o.renderOrder=2;}});hybridOverlay.visible=false;scene.add(hybridOverlay);resolve();},undefined,reject)).catch(e=>{hybridOverlayPromise=null;toast('보완 영역 표시를 불러오지 못했습니다.');console.error(e);});
 await hybridOverlayPromise;if(hybridOverlay)hybridOverlay.visible=wanted();
}
$('#hybridHighlight').onchange=updateHybridOverlay;
function loadScan(){const asset=mvsAsset;if(mvsCache.has(asset))return mvsCache.get(asset);const pending=new Promise((resolve,reject)=>loader.load(asset+(asset==='mvs-hybrid.glb'?'?v=2':''),g=>{const model=g.scene;model.traverse(o=>{if(o.isMesh){const old=o.material;o.material=new THREE.MeshBasicMaterial({map:old.map,vertexColors:!!o.geometry.attributes.color,side:THREE.DoubleSide,clippingPlanes:$('#cutaway').checked?[roofClip]:[]});}});model.visible=false;scene.add(model);resolve(model);},undefined,reject)).catch(e=>{mvsCache.delete(asset);throw e;});mvsCache.set(asset,pending);return pending;}
...
$('#mvsControls').hidden=mode!=='scan';$('#hybridHighlightRow').hidden=mvsAsset!=='mvs-hybrid.glb';$('#mvsCompareNote').textContent=mvsAsset==='mvs-hybrid.glb'?'기존 MVS 표면을 유지하고 여러 시점에서 검증한 LingBot 예측으로 빈 곳을 보완했습니다.':'같은 깊이맵에서 표면 융합 조건을 비교합니다. 큰 미관측 영역은 남아 있습니다.';updateHybridOverlay();$('#mapControls').hidden=mode!=='lingbot';$('#modeNote').textContent={cute:'기존 복원 좌표를 기준으로 가구를 다듬은 모델입니다.',scan:'SfM/MVS로 복원한 표면 모델입니다. 다른 탭과 같은 시점으로 비교할 수 있습니다.',lingbot:'여러 시점에서 구성한 점군입니다. MVS 모델과 같은 좌표에 정렬했습니다.'}[mode];
if(mode==='scan'&&mvsAsset==='mvs-hybrid.glb')$('#modeNote').textContent='MVS 기반 표면에 LingBot 예측으로 누락 영역을 보완한 혼합 복원입니다.';$('#modePill').textContent=(mode==='scan'&&mvsAsset==='mvs-hybrid.glb'?'MVS + LingBot':names[mode])+(mode==='lingbot'?'':' · 약 4 m 폭');$('#modelStatus').textContent='';resize();}
...
try{if(next==='scan'){const loaded=await loadScan();if(ticket===requestNumber){if(scan)scan.visible=false;scan=loaded;scan.traverse(o=>{if(o.isMesh)o.material.clippingPlanes=$('#cutaway').checked?[roofClip]:[];});}}if(next==='lingbot')await loadMap();if(ticket===requestNumber)applyMode(next);}catch(e){if(ticket===requestNumber){$('#modelStatus').textContent='';toast('모델 파일을 불러오지 못했습니다. 로컬 웹 주소에서 다시 시도해 주세요.');}console.error(e);}}
