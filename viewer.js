...
const mvsCache=new Map();let mvsAsset='mvs-balanced.glb';
const mvsPanel=document.createElement('section');mvsPanel.id='mvsControls';mvsPanel.hidden=true;
mvsPanel.innerHTML='<label for="mvsVariant">MVS 비교</label><select id="mvsVariant" style="width:100%;padding:9px;border:1px solid #dde3de;border-radius:9px;background:#f7f8f4;color:#4e625d"><option value="mvs.glb">기존</option><option value="mvs-balanced.glb" selected>보완 · 중간 완화</option><option value="mvs-complete.glb">비교 · 강한 완화</option></select><p style="font-size:11px;line-height:1.6;color:#7d8580">같은 깊이맵에서 표면 융합 조건을 비교합니다. 큰 미관측 영역은 남아 있습니다.</p>';
...
$('#mvsControls').hidden=mode!=='scan';$('#mapControls').hidden=mode!=='lingbot';$('#modeNote').textContent={cute:'기존 복원 좌표를 기준으로 가구를 다듬은 모델입니다.',scan:'SfM/MVS로 복원한 표면 모델입니다. 다른 탭과 같은 시점으로 비교할 수 있습니다.',lingbot:'여러 시점에서 구성한 점군입니다. MVS 모델과 같은 좌표에 정렬했습니다.'}[mode];
