import { installMacBook } from './macbook.js?v=macbook-motion-2';
...
let dalgu,fridge,tv,monitorHome,macbook;
...
function animate(){requestAnimationFrame(animate);const now=performance.now(),dt=(now-lastFrame)/1000;lastFrame=now;if(tourPlaying)tickTour(dt);else if(!dalgu?.active)controls.update();dalgu?.tick(dt);fridge?.tick(dt);macbook?.tick(dt);cutaway();tv?.tick(dt);monitorHome?.tick(dt);renderer.render(scene,camera);}animate();
window.labViewer={scene,camera,renderer,controls,preset,selectModel,get mode(){return mode;},get cute(){return cute;},get fridge(){return fridge;},get macbook(){return macbook;},get tv(){return tv;},get monitorHome(){return monitorHome;},get scan(){return scan;},get lingbot(){return lingbot;},get gs(){return extraGS.get(gsVariant)?.mesh||(gsVariant==='surface'?surfaceGS:gs);},get gsVariant(){return gsVariant;}};
