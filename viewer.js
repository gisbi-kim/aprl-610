...
const mvsCache=new Map();let mvsAsset='mvs-lingbot-larger.glb';
const mvsPanel=document.createElement('section');mvsPanel.id='mvsControls';mvsPanel.hidden=true;
mvsPanel.innerHTML='<label for="mvsVariant">MVS 비교</label><select id="mvsVariant" style="width:100%;padding:9px;border:1px solid #dde3de;border-radius:9px;background:#f7f8f4;color:#4e625d"><option value="mvs-lingbot-larger.glb" selected>LingBot points 기반 · 19.00 MB</option><option value="mvs.glb">SfM/MVS 기반 · 11.22 MB</option></select><label style="display:block;margin-top:12px">채도 <output id="mvsSaturationValue">65%</output><input id="mvsSaturation" type="range" min="0" max="1.2" step="0.05" value="0.65" style="width:100%"></label><label id="hybridHighlightRow" hidden><input type="checkbox" id="hybridHighlight">LingBot 보완 부분 표시</label><p id="mvsCompareNote" style="font-size:11px;line-height:1.6;color:#7d8580">같은 깊이맵에서 표면 융합 조건을 비교합니다. 큰 미관측 영역은 남아 있습니다.</p>';
