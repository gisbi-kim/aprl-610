...
export async function installDalgu(viewer){
  ...
  panel.innerHTML='<hr><div class="label">달구와 산책</div><div class="dalgu-buttons"><button id="dalguEyes">달구 시점</button><button id="dalguFollow">따라가기</button><button id="dalguExit" hidden>둘러보기로</button></div><p id="dalguSize">키 1.0 m</p><p id="dalguHelp">WASD · 방향키 이동<br>화면 클릭 후 마우스로 시선 조절 · Esc 해제</p><p id="dalguJumpHelp">Space 점프 · 공중에서 한 번 더 누르면 2단 점프</p><button id="dalguJump" class="wide" disabled>점프 · 2단 점프</button><button id="dalguReset" class="wide">달구 위치 초기화</button>';
  document.querySelector('aside').append(panel);
  const style=document.createElement('style');style.textContent='#dalguPanel{grid-column:1/-1}#dalguPanel .label{margin-top:12px}.dalgu-buttons{display:flex;gap:6px;flex-wrap:wrap}.dalgu-buttons button{flex:1;white-space:nowrap}#dalguSize{font-size:11px;color:#738479;margin:12px 0}#dalguJumpHelp{font-size:11px;color:#738479;line-height:1.7}#dalguHelp{font-size:11px;line-height:1.7;color:#738479}body.dalgu-walk #viewNavigator,body.dalgu-walk .views{opacity:.5}';document.head.append(style);
  const $=id=>document.getElementById(id),keys=new Set();
  let active=false,follow=false,yaw=0,pitch=0,phase=0,saved=null,drag=null;
  const height=1.0;
  const groundY=.016,gravity=9.8,jumpSpeed=3.6;let verticalSpeed=0,jumpCount=0;
  function jump(){if(!active||jumpCount>=2)return false;verticalSpeed=jumpSpeed;jumpCount++;return true;}
  ...
  window.addEventListener('keydown',e=>{if(!active||e.target.closest?.('input,button,select,textarea'))return;if(e.code==='Space'){e.preventDefault();if(!e.repeat)jump();return;}if(moving.includes(e.code)){e.preventDefault();keys.add(e.code);}});
  ...
  reset();viewUpdate();
  return {tick,enter,exit,reset,blocked,jump,get jumpCount(){return jumpCount;},avatar:avatar.root,get active(){return active;},get position(){return position.clone();},get height(){return height;}};
}
